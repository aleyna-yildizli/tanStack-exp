...
export default function Contact() {
  ...
  useEffect(()=>{
    queryClient.invalidateQueries(['getContact', contactId]);
  },[contactId])
  ...
  const mutation = useMutation({
    mutationFn: (contactId) =>
      axios
        ...
        .then((res) => res.data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["sideBarUserList"] });//sidebardaki kullanıcı listesini güncelle
      history.push("/");
    },
  });
  ...
  return (
    <div id="contact">
      ...
      <div>
        ...
        <div>
          <button disabled={mutation.isPending} className="delete" onClick={handleDelete}>
            Delete
          </button>
        </div>
      </div>
